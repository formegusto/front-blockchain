...
contract("Erc20Token Test", async (accounts) => {
  ...
  it("The deployer has 10 tokens", async () => {
    let deployerBalance = await erc20TokenInstance.balanceOf(accounts[0]);
  ...
  it("1 token is sent to account1", async () => {
    ...
    let deployerBalance = await erc20TokenInstance.balanceOf(accounts[0]);
    let accountBalance = await erc20TokenInstance.balanceOf(accounts[1]);

    expect(deployerBalance).to.be.bignumber.equal(
      web3.utils.toWei("9", "ether")
    );
    expect(accountBalance).to.be.bignumber.equal(
      web3.utils.toWei("1", "ether")
    );
  });
});
